...
namespace Financial_Accounting_Blazor.Controllers
{
    public class GetOperationsUseCase
    {
        ...
        BalanceContextDb db;
        public GetOperationsUseCase(IUnitOfWork unitOfWork)
        {
        ...
        public string Execute(DateTime start, DateTime end)
        {
            ...
            List<Transaction> dbResult  = this.unitOfWork.GetAll().
                                                               Where(x =>
                                                               (Int64)TimeSpan.FromTicks(x.DateTime.Ticks).TotalSeconds >= toCompareStart &&
                                                               (Int64)TimeSpan.FromTicks(x.DateTime.Ticks).TotalSeconds <= toCompareEnd).
                                                               ToList();
            ResultBuilder resultBuilder = new ResultBuilder();
            this.unitOfWork.Save();
            return JsonSerializer.Serialize<UseCaseResult>(resultBuilder.Build(dbResult));
